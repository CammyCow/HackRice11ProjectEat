...
data_set = pd.read_csv('D:/Desktop/hackrice/test_data/ratings_small.csv')
...
#popularity model
freq_set = data_set['movieId'].value_counts().to_frame().reset_index()
freq_set.columns = ['movieId','freq']
sum_set = data_set.groupby('movieId')['rating'].sum().sort_values(ascending=False).reset_index()
sum_set.columns = ['movieId','sum']
item_popularity_set = freq_set.set_index('movieId').join(sum_set.set_index('movieId')).reset_index()
item_popularity_set['average'] = item_popularity_set['sum']/item_popularity_set['freq']
C = item_popularity_set['average'].mean()
minimum_vote = item_popularity_set['freq'].sum()*0.1
item_popularity_set['weighted'] = (item_popularity_set['freq']/(item_popularity_set['freq'] + minimum_vote)*item_popularity_set['average']) + (minimum_vote/(minimum_vote+item_popularity_set['freq'])*C)
...
class PopularityRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10):
        recommendation_df = self.popularity_df[~self.popularity_df['movieId'].isin(items_to_ignore)].sort_values('weighted', ascending = False).head(topn)
        recommendation_df = recommendation_df.drop(['freq', 'sum','average'], axis=1)
        recommendation_df['weighted'] = (recommendation_df['weighted']-recommendation_df['weighted'].min())/(recommendation_df['weighted'].max()-recommendation_df['weighted'].min())
